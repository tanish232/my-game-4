...
if(frameCount%168===0){
alien2=createSprite(Math.round(random(displayWidth)),displayHeight/2-450,50,50)  
...
if(frameCount%172===0){
  alien3=createSprite(Math.round(random(displayWidth)),displayHeight/2-450,50,50)  
  ...
  if(frameCount%160===0){
    alien4=createSprite(Math.round(random(displayWidth)),displayHeight/2-450,50,50)  
    ...
    if(frameCount%157===0){
      alien5=createSprite(Math.round(random(displayWidth)),displayHeight/2-450,50,50)  
...
if(laserGroup.isTouching(alien2Group)){
 // laserGroup.destroyEach();
  alien2Group.destroyEach();  
  score=score+1;  
}
  if(laserGroup.isTouching(alien3Group)){
  //  laserGroup.destroyEach();
    alien3Group.destroyEach();  
    score=score+1;  
  }
  if(laserGroup.isTouching(alien4Group)){
   // laserGroup.destroyEach();
    alien4Group.destroyEach();  
    score=score+1;  
  }
  if(laserGroup.isTouching(alien5Group)){
   // laserGroup.destroyEach();
